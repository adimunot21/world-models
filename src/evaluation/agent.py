...
class WorldModelAgent:
    def __init__(self, vae, mdn_rnn, controller, device, img_size=64):
        ...
        self.hidden = None; self.prev_z = None; self.prev_action = None

    def reset(self):
        self.hidden = self.mdn_rnn.init_hidden(1, self.device)
        self.prev_z = None; self.prev_action = None
    ...
    def act(self, observation):
        ...
        if self.prev_z is not None and self.prev_action is not None:
            _, _, _, _, self.hidden = self.mdn_rnn.predict_next(
                self.prev_z, self.prev_action, self.hidden
            )
        h = self.hidden[0][-1]
        action = self.controller(z, h)
        action_np = action.squeeze(0).cpu().numpy()
        self.prev_z = z; self.prev_action = action
        return action_np

def load_agent(vae_checkpoint="checkpoints/vae/best.pt",
               mdn_rnn_checkpoint="checkpoints/mdn_rnn/best.pt",
               controller_params=None, device=None):
    ...
    mdn_rnn = MDNRNN(latent_dim=cfg.get("latent_dim", 32), action_dim=cfg.get("action_dim", 3),
                      hidden_dim=cfg.get("hidden_dim", 256), num_gaussians=cfg.get("num_gaussians", 5))
    mdn_rnn.load_state_dict(rnn_ckpt["model_state_dict"])
    controller = Controller(latent_dim=latent_dim, hidden_dim=cfg.get("hidden_dim", 256), action_dim=3)
    if controller_params is not None: controller.set_flat_params(controller_params)
...
def evaluate_agent(agent, num_episodes=5, max_steps=1000, render=False):
    ...
    for ep in range(num_episodes):
        obs, _ = env.reset(); agent.reset(); total_reward = 0.0
    ...
    env.close()
    return {"rewards": rewards, "mean_reward": np.mean(rewards),
            "std_reward": np.std(rewards), "num_episodes": num_episodes}
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Reward: {results['rewards'][0]:.1f}")
